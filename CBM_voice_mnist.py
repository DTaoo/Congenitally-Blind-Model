...
class C_Blind(object):
    ...
    def build_audio_D(self):
        ...
        spectro = logMelSpectrogram()(audio_input)
        ...
        #fea = Dense(self.audio_emb_dim, activation='relu', name='embeddings')(x)

        valid = Dense(1, activation='sigmoid', name='predictions')(fea)
        labels = Dense(11, activation='softmax')(fea)
    ...
    def test_imagine(self, audio_embeddings, labels, epoch):
        ...
        gen_imgs = img_extractor.predict_on_batch([rand_embeddings, noise])

        for i in range(self.batch_size):
            img_name = 'gen_img_%d_%d_label_%d.jpg' % (epoch, i, rand_labels[i])
            x = np.squeeze(gen_imgs[i]) * 255
            img = Image.fromarray(x)
            img = img.convert('L')
            img.save('gen_mnist/' + img_name)
    ...
    def train(self, epochs=100):
        ...
        name = 'audio_embeddings/audio_embedding_mnist_train_15.npy'
        train_embeddings = np.load(name)
 
        name = 'audio_embeddings/audio_embedding_mnist_test_15.npy'
        ...
        for i in range(100):
            img_name = 'ori_img_%d.jpg' % i
            im = Image.fromarray(np.squeeze(imagine_imgs[i] * 255))
            im = im.convert('L')
            im.save('gen_mnist/' + img_name)
        ...
        for epoch in range(epochs):
            ...
            print("%d [D loss: %.3f, acc: %.2f, class_acc: %.2f] [G loss: %.3f, acc: %.2f, class_acc: %.2f]" % (epoch, d_loss[0], 100 * d_loss[3], 100 * d_loss[4], g_loss[0], 100 * g_loss[3], 100 * g_loss[4]))
